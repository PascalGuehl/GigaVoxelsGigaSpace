...
//	mGraphicsSynthesizer->setClearColor( 255.f * clear_color.x, 255.f * clear_color.y, 255.f * clear_color.z, 255.f * clear_color.w );
	...
	//	mGraphicsSynthesizer->execute();
		...
		glm::mat4 m = glm::make_mat4( mGraphicsSynthesizer->_modelMatrix._array );
		...
		float4x4 MV;
		float4x4 P;
		//memset(MV._array, 0, 16 * sizeof(float));
		//memset(P._array, 0, 16 * sizeof(float));
		//int4 Viewport;
		//for ( int i = 0; i < 16; i++ )
		int k = 0;
		for ( int c = 0; c < 4; c++ )
		...
		//float4x4 model;
		//memset(model._array, 0, 16 * sizeof(float));
		//// scale
		//model._array[0] = 1.0f;
		//model._array[5] = 1.0f;
		//model._array[10] = 1.0f;
		//model._array[15] = 1.0f;
		//// translation
		//model._array[12] = -0.5f;
		//model._array[13] = -0.5f;
		//model._array[14] = -0.5f;
		int4 viewport;
		GLint last_viewport[4];
		glGetIntegerv( GL_VIEWPORT, last_viewport) ;
		viewport = make_int4( last_viewport[0], last_viewport[1], last_viewport[2], last_viewport[3] );
		mGraphicsSynthesizer->render( mGraphicsSynthesizer->_modelMatrix, MV, P, viewport );
#else
